...
def fused_recurrent_gated_delta_rule_fwd_cpu(
    ...
    return o.to(torch.float16), final_state.to(torch.float16)
...
class TestFusedRecurrentDeltaRule(unittest.TestCase):
    ...
    def test_varlen(self):
        ...
        q = torch.randn(B, total_T, H, K, dtype=torch.float16) 
        k = torch.randn(B, total_T, H, K, dtype=torch.float16) 
        v = torch.randn(B, total_T, HV, V_val, dtype=torch.float16) 
        g = -0.5 + torch.rand(B, total_T, HV, dtype=torch.float16) * 0.1
        beta = torch.sigmoid(torch.randn(B, total_T, HV, dtype=torch.float16))
        scale = K ** -0.5
        N = len(cu_seqlens) - 1
        h0 = torch.randn(N, HV, V_val, K, dtype=torch.float16)
