...
from chunk_fwd_o import chunk_fwd_o as chunk_fwd_o_triton,  prepare_chunk_indices
        ...
        for i_t in range(NT):
            ...
            for h_idx in range(H):
                ...
                if g is not None:
                    ...
                    o_chunk = o_chunk * torch.exp(g_chunk[:, None])
                    A_chunk = A_chunk * torch.exp(g_chunk[:, None] - g_chunk[None, :])
...
class TestChunkFwdO(unittest.TestCase):
    def test_basic(self):
        print("\n=== Basic ===")
    ...
    def test_multi_head(self):
        ...
        o_diff = (o_cpu.float() - o_triton.cpu().float()).abs().max().item()
        print(f"Multi-head - o max diff: {o_diff:.6f}")
        self.assertLess(o_diff, 1e-3)

    def test_large(self):
        print("\n=== Large BT=64 multi-head ===")
        B, T, Hg, K, H, V = 1, 128, 2, 128, 4, 128
        BT = 64
        ...
        o_diff = (o_cpu.float() - o_triton.cpu().float()).abs().max().item()
        print(f"Large - o max diff: {o_diff:.6f}")
        self.assertLess(o_diff, 1e-3)
