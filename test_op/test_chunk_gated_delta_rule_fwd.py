...
def chunk_gated_delta_rule_fwd_cpu(
    ...
    # Step 3: solve_tril
    A = solve_tril_cpu_varlen(
        A=A, cu_seqlens=cu_seqlens, chunk_indices=chunk_indices, BT=chunk_size,
    )

    # Step 4: recompute_w_u
    w, u = recompute_w_u_cpu_varlen(
        k=k, v=v, beta=beta, g_cumsum=g_cumsum, A=A,
        cu_seqlens=cu_seqlens, chunk_indices=chunk_indices,
    )

    # Step 5: chunk_gated_delta_rule_fwd_h
    h, v_new, final_state = chunk_gated_delta_rule_fwd_h_cpu_varlen(
